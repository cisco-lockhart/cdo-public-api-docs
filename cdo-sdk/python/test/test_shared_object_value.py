# coding: utf-8

"""
    API Docs

    Use the interactive documentation to explore the endpoints CDO has to offer

    The version of the OpenAPI document: 0.0.1
    Contact: cdo.tac@cisco.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.shared_object_value import SharedObjectValue

class TestSharedObjectValue(unittest.TestCase):
    """SharedObjectValue unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SharedObjectValue:
        """Test SharedObjectValue
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SharedObjectValue`
        """
        model = SharedObjectValue()
        if include_optional:
            return SharedObjectValue(
                object_type = 'NETWORK_OBJECT',
                default_content = {"literal":"a:b:c::1"},
                overrides = [
                    {"targetId":"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","content":"a:b:c::2"}
                    ]
            )
        else:
            return SharedObjectValue(
                object_type = 'NETWORK_OBJECT',
                default_content = {"literal":"a:b:c::1"},
        )
        """

    def testSharedObjectValue(self):
        """Test SharedObjectValue"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
